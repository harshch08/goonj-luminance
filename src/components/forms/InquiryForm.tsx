...
import { Send, CheckCircle, Sparkles, Star, IndianRupee } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { useToast } from '@/hooks/use-toast';
import { useCelebrityArtists } from '@/hooks';
import {
  Select,
  SelectContent,
  SelectItem,
...
export const InquiryForm = ({ serviceType, compact = false, hideEventType = false, preSelectedCelebrity }: InquiryFormProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    specificService: '',
    celebrityArtist: preSelectedCelebrity || '',
    message: '',
  });

  // Fetch celebrity artists for Celebrity Concerts service
  const { data: celebrityArtists } = useCelebrityArtists();
  const selectedCelebrity = celebrityArtists?.find(a => a.name === formData.celebrityArtist);
  ...
  const isCelebrityService = formData.eventType === 'Celebrity Concerts' || serviceType === 'Celebrity Concerts';
  ...
  useEffect(() => {
    if (serviceType) {
      setFormData(prev => ({
        ...
        celebrityArtist: preSelectedCelebrity || '' // Use pre-selected celebrity if provided
      }));
    }
  }, [serviceType, preSelectedCelebrity]);

  // Update celebrity selection when preSelectedCelebrity changes
  useEffect(() => {
    if (preSelectedCelebrity) {
      setFormData(prev => ({
        ...prev,
        celebrityArtist: preSelectedCelebrity
      }));
    }
  }, [preSelectedCelebrity]);
...
${formData.celebrityArtist ? `*Celebrity Artist:* ${formData.celebrityArtist}` : ''}
${selectedCelebrity ? `*Package:* ${selectedCelebrity.package}` : ''}
...
${formData.message ? `\n*Message:*\n${formData.message}` : ''}
    ...
    setTimeout(() => {
      ...
      setFormData({ name: '', email: '', phone: '', eventType: serviceType || '', specificService: '', celebrityArtist: '', message: '' });
    }, 3000);
  };
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div className={`grid gap-6 ${compact ? '' : 'md:grid-cols-2'}`}>
          ...
          {currentServiceOptions.length > 0 && !isCelebrityService && (
          ...
          {/* Celebrity Artist Selection - Only for Celebrity Concerts */}
          {isCelebrityService && celebrityArtists && celebrityArtists.length > 0 && (
            <div className={`space-y-2 ${compact ? '' : 'md:col-span-2'}`}>
              <label className="text-sm font-medium text-foreground/80 uppercase tracking-wide flex items-center gap-2">
                <Star className="w-4 h-4 text-gold" />
                Select Celebrity Artist
              </label>
              <Select
                value={formData.celebrityArtist}
                onValueChange={(value) => setFormData(prev => ({ ...prev, celebrityArtist: value }))}
              >
                <SelectTrigger className="h-12 bg-background/50 border-border/30 focus:border-gold/50 focus:ring-gold/20 text-foreground transition-all duration-300 hover:border-gold/30">
                  <SelectValue placeholder="Choose a celebrity artist" />
                </SelectTrigger>
                <SelectContent className="bg-background/95 backdrop-blur-md border-border/50 max-h-[300px]">
                  {celebrityArtists.map((artist) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">({artist.niche})</span>
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          {selectedCelebrity && (
            ...
            >
              <div className="relative overflow-hidden rounded-lg border-2 border-gold/30 bg-gradient-to-br from-gold/10 via-gold-light/5 to-gold/10 p-6">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="font-display text-lg font-bold text-foreground">Estimated Quotation</h4>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="font-semibold text-foreground text-lg">{selectedCelebrity.name}</p>
                        <p className="text-sm text-muted-foreground">{selectedCelebrity.niche}</p>
                      </div>
                      <div className="px-3 py-1 rounded-full text-xs font-bold uppercase bg-gold/20 text-gold-light">
                        Celebrity
                      </div>
                    </div>
                    <div className="pt-3 border-t border-gold/20">
                      <p className="text-sm text-muted-foreground mb-1">Package Details:</p>
                      <p className="text-base font-semibold text-gold-light">{selectedCelebrity.package}</p>
                    </div>
                    <div className="flex items-center gap-2 text-xs text-muted-foreground pt-2">
                      <Sparkles className="w-3 h-3" />
                      <span>Final pricing may vary based on event requirements</span>
                    </div>
                  </div>
                </div>
              </div>
